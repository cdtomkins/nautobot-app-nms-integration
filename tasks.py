...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_NMS_INTEGRATION_xxx
namespace = Collection("nautobot_nms_integration")
namespace.configure(
    {
        "nautobot_nms_integration": {
            "nautobot_ver": "2.3.1",
            "project_name": "nautobot-nms-integration",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_nms_integration.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_nms_integration.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_nms_integration.nautobot_ver,
        "PYTHON_VER": context.nautobot_nms_integration.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_nms_integration.project_name}",
        f'--project-directory "{context.nautobot_nms_integration.compose_dir}"',
    ]

    for compose_file in context.nautobot_nms_integration.compose_files:
        compose_file_path = os.path.join(context.nautobot_nms_integration.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_nms_integration.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_nms_integration.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_nms_integration.python_ver
    dockerfile_path = os.path.join(context.nautobot_nms_integration.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_nms_integration.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_nms_integration.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.nautobot_nms_integration.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_nms_integration"
...
@task(
    help={
        "version": "Version of NMS Integration to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_nms_integration"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_nms_integration") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_nms_integration.migrations"
        )
...
def unittest(  # noqa: PLR0913
    context,
    keepdb=False,
    label="nautobot_nms_integration",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_nms_integration/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_nms_integration.local):
